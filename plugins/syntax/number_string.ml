...
let locate_global_inductive_or_pstring_or_int63 env allow_params qid =
  try TargetInd (locate_global_inductive_with_params allow_params qid)
  with Not_found ->
    ...
    else if allow_params && Coqlib.has_ref int63n
       && Environ.QGlobRef.equal env (Smartlocate.global_with_alias qid) (Coqlib.lib_ref int63n)
    then TargetPrim (Coqlib.lib_ref int63w, [Coqlib.lib_ref int63c],
                     (Nametab.path_of_global (Coqlib.lib_ref int63n), []))
    else TargetInd (Smartlocate.global_inductive_with_alias qid, [])
...
let locate_int63 () =
  Option.map gref (Coqlib.lib_ref_opt "num.int63.type")

let type_error_to f ty =
  CErrors.user_err
    (pr_qualid f ++ str " should go from Byte.byte, (list Byte.byte), PString.string or int to " ++
     pr_qualid ty ++ str " or (option " ++ pr_qualid ty ++ str ").")

let type_error_of g ty =
  CErrors.user_err
    (pr_qualid g ++ str " should go from " ++ pr_qualid ty ++
     str " to T or (option T), where T is either Byte.byte, (list Byte.byte), Pstring.string, or int.")
...
let vernac_string_notation local ty f g via scope =
  ...
  let ty, via =
    match via with None -> ty, via | Some (ty', a) -> ty', Some (ty, a) in
  let tyc_params = locate_global_inductive_or_pstring_or_int63 env (via = None) ty in
  let to_ty = Smartlocate.global_with_alias f in
  let of_ty = Smartlocate.global_with_alias g in
  let f_name, f = f, intern_cref env sigma f in
  let g_name, g = g, intern_cref env sigma g in
  ...
  let to_kind =
    ...
    | _ ->
    match int63_ty with
    | Some int63 when has_type env sigma g (arrow int63 cty) -> IntCharCode, Direct
    | Some int63 when has_type env sigma g (arrow int63 (opt cty)) -> IntCharCode, Option
    | _ ->
  ...
  let of_kind =
    ...
    | Some pstring when has_type env sigma g (arrow (opt cty) pstring) -> PString, Option
    | _ ->
    match int63_ty with
    | Some int63 when has_type env sigma g (arrow cty int63) -> IntCharCode, Direct
    | Some int63 when has_type env sigma g (arrow (opt cty) int63) -> IntCharCode, Option
    | _ ->
