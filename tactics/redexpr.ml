...
let reduction_of_red_expr_val = function
  ...
  | ReRed(w,f) ->
    let rered = ReRed.rered w in
    (e_red (rered f),DEFAULTcast)
  | Lazy (w,f) ->
...
let bind_red_expr_occurrences occs nbcl redexp =
  ...
  if occs == AllOccurrences then
    ...
    | Red | Hnf | Cbv _ | Lazy _ | Cbn _ | ReRed _
...
module Intern = struct
  ...
  let intern_red_expr ist = function
    ...
    | ReRed f -> ReRed (intern_flag ist f)
...
module Interp = struct
  ...
  let interp_red_expr ist env sigma = function
    ...
    | ReRed f -> sigma , ReRed (interp_flag ist env sigma f)
